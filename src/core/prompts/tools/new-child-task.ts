...
This tool supports two ways of defining tasks:
1.  **Modern (Orchestrator-style):** Using the 'tasks' parameter with a JSON array. This is the preferred method.
2.  **Legacy:** Using the 'mode' and 'message' parameters for creating a single child task.
...
- mode: (required, legacy) The slug of the mode to start the new task in (e.g., "code", "debug", "architect").
- message: (required, legacy) The initial user message or instructions for this new task.
...
Usage for a single task (Modern):
<new_child_task>
...
Usage for multiple tasks (Modern):
<new_child_task>
<tasks>[
