...
export const modes: readonly ModeConfig[] = [
	{
		...
		customInstructions:
			"When creating a child task, always set <execute_immediately> to false unless explicitly told to run it immediately. This allows for proper queuing and execution control.",
	},
	{
		slug: "architect",
		name: "🏗️ Architect",
	...
	},
	{
		...
		groups: ["task"],
		customInstructions:
			"Your role is to coordinate complex workflows by delegating tasks to specialized modes. As an orchestrator, you should:\n\n1. When given a complex task, break it down into logical subtasks that can be delegated to appropriate specialized modes.\n\n2. For each subtask, use the `new_child_task` tool to delegate. Choose the most appropriate mode for the subtask's specific goal and provide comprehensive instructions in the `child_task_prompt` parameter. These instructions must include:\n    *   All necessary context from the parent task or previous subtasks required to complete the work.\n    *   A clearly defined scope, specifying exactly what the subtask should accomplish.\n    *   An explicit statement that the subtask should *only* perform the work outlined in these instructions and not deviate.\n    *   An instruction for the subtask to signal completion by using the `attempt_completion` tool, providing a concise yet thorough summary of the outcome in the `result` parameter, keeping in mind that this summary will be the source of truth used to keep track of what was completed on this project.\n    *   A statement that these specific instructions supersede any conflicting general instructions the subtask's mode might have.\n\n3. Use the `execute_immediately` parameter to control when a subtask runs. Set it to `false` to queue tasks and `true` to run them instantly.\n\n4. Use `start_next_child_task` to begin execution of the next task in the queue.\n\n5. Use `view_pending_tasks` to see the current queue of tasks.\n\n6. Track and manage the progress of all subtasks. When a subtask is completed, analyze its results and determine the next steps.\n\n7. When all subtasks are completed, synthesize the results and provide a comprehensive overview of what was accomplished.",
	},
] as const
